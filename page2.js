...
const handOptions = {
 ...
 if(pScore == roundsleft){
  localStorage.setItem("camefrom",1);
 ...
 function playerlose(language){
  ...
  const bChoices = ["rock","paper","scissors"];
  ...
  function startgame(Choice){
    ...
    const arCh = [ "حجر","ورقة","مقص"]
    const grCh = ["Felsen","Papier","Schere"]
    const krCh = ["바위","종이","가위"]
    var index;
    var index2;
    ...
    if((Choice == "rock"&& bChoice=="scissors")||(Choice == "scissors"&& bChoice=="paper") ||(Choice == "paper"&& bChoice=="rock")){
